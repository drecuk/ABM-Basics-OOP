...
//
//  Agent Interaction: predator-prey seek and chase system
//
//  ----------------------------------------------------------
//  How to compile:
//  note that we are now using both SDL2 and OpenGL, thus the -l for all libraries
//  we are also using multiple cpp files
//  sudo g++ -I/usr/include/ main.cpp Agent.cpp Predator.cpp Prey.cpp Grid.cpp -o main -L/usr/lib -lSDL2 -lGL -lGLU
//
...
int main(int argc, char**argv)
{
    ...
    int agentNo = 4;
    int agentNoSplit = 2; // 2 types of agents (predator and prey)
    ...
    // derived classes
    Predator **predators = new Predator*[agentNo/agentNoSplit];
    Prey **preys = new Prey*[agentNo/agentNoSplit];

    // 1st Loop: create agents types first
    cout<<"----- Creating Types"<< endl;
    for(int i=0; i<agentNo; i++)
    {
      if(i<agentNo/agentNoSplit)
      {
        predators[i] = new Predator(i, 0, 0, 0, 0.001f);

        //predators[i]->chase();
      }
      else if (i>=(agentNo/agentNoSplit))
      {
        //cout<<"creating types dogs:"<<i-5<<endl;
        preys[i-(agentNo/agentNoSplit)] = new Prey(i, -10, 0, 10, 0.001f);

        //preys[i-(agentNo/agentNoSplit)]->chase();
      }
    }
    ...
    for(int i=0; i<agentNo; i++)
    {
      cout<<agentNo/agentNoSplit<<endl;
      if(i<agentNo/agentNoSplit)
      {
      ...
      {
        cout << "-- deriving preys["<<i<<"] "<<preys[i-(agentNo/agentNoSplit)]<<endl;
        agents[i] = preys[i-(agentNo/agentNoSplit)];
        agents[i]->speciesType = PREY;
        //agents[i]->chase();
      }
    }

    cout << "----- Getting pointer to all agents" << endl;
    for(int i=0; i<agentNo; i++)
    {
